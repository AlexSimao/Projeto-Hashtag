...
export const criarCard = () => {
  for (let produto of produtos) {
    const cardProduto = `
      ...
      produto.female ? "feminino" : "masculino"
    }">
            ...
            <button id="btn-add-produto-${
              produto.id
            }" class="bg-emerald-950 text-white active:text-stone-400 hover:bg-emerald-900"><i class="fa-solid fa-cart-plus"></i></button>
        </div>`;
  ...
  for (let produto of produtos) {
    document
      .querySelector(`#btn-add-produto-${produto.id}`)
